...
# Extract model, directory, cwd, and effort level
...
if [[ -n "$five_pct" || -n "$week_pct" ]]; then
    ...
    rate_limits=" | ${C_GRAY}${rate_parts}"
fi

# Build effort level indicator
effort_display=""
if [[ -n "$effort" ]]; then
    case "$effort" in
        low)    effort_display=" ⚡lo" ;;
        medium) effort_display=" ⚙️ med" ;;
        high)   effort_display=" 🔥 hi" ;;
    esac
fi

# Build output: Model | Dir | Branch (uncommitted) | Context | Rate Limits
output="${C_ACCENT}${model}${effort_display}${C_GRAY} | 📁 ${dir}"
[[ -n "$branch" ]] && output+=" | 🔀 ${branch} ${git_status}"
output+=" | ${ctx}${rate_limits}${C_RESET}"
